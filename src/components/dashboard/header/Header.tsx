...
export default function Header({ handleLogout, user }: any) {
  ...
  return (
    <div className="w-full min-h-[30px] relative flex items-center justify-between bg-[#12604a] p-[20px] py-[15px]">
      <Image src="https://i.imgur.com/jk1lAix.png" width={200} height={200} className="w-[125px] h-[25px]" alt="logo" />
      ...
      <div className="relative">
        ...
        {open && (
          <div className="absolute mt-1 z-50 shadow bg-white border py-2 rounded-md right-0 flex flex-col justify-center gap-[5px]">
            <p className="text-[14px] px-[15px] py-[5px] whitespace-nowrap font-medium text-[#12604a]">
              {user.holder.fullName}&nbsp;{user.holder.lastName}
            </p>
            <p className="text-[14px] m-1 px-[15px] text-center rounded-md py-[5px] bg-[#12604a] border whitespace-nowrap text-white" onClick={handleLogout}>
              Sign out
            </p>
          </div>
        )}
